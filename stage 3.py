...
ps = []
for x in xs:
    pr = u1_r(Point(x, H/2.0))
    pi = u1_i(Point(x, H/2.0))
    ps.append(math.hypot(pr, pi))
ps = np.array(ps)
...
isol = []
print("Gap(um), P1, P2, Isolation(dB)")
for gap in gaps:
    ...
    P1 = math.hypot(u2_r(Point(L, H/2.0)), u2_i(Point(L, H/2.0)))
    P2 = math.hypot(u2_r(Point(L, H+gap+H/2.0)), u2_i(Point(L, H+gap+H/2.0)))
    iso = 20.0 * math.log10(P2/P1) if P1 > 0 else -np.inf
    isol.append(iso)
    print("{:.1f}, {:.3e}, {:.3e}, {:.2f}".format(gap*1e6, P1, P2, iso))
...
num_holes = int(Lx / (0.5e-3))
for i in range(num_holes):
    geom3 -= Circle(Point(0.5e-3 + i*0.5e-3, 0.0), 0.5e-3)
