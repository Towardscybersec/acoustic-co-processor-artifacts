...
Enhanced acoustic FEM simulations (FEniCS 2016.2.0 + mshr):
 ...
 2) Crosstalk vs. gap with a 4-layer 3D stacked phononic barrier (128 cells, tapered interfaces, tungsten pillars, local absorbers)
 3) Metamaterial spatial FFT (zero-padding, high resolution, bandgap width & frequency mapping)
...
# ─────────────────────────────────────────────────────────────────────────────
# Physical & material parameters (SI units)
...
# Domain parameters
domain_length    = 1e-3    # [m] (for attenuation)
...
layers            = 4           # stacked lattices
rows_per_layer    = 64          # rows per lattice
columns           = 128         # cells in each lattice
taper_count       = 16          # taper over 16 cells
radius_factor     = 0.50        # hole radius factor
res_radius_factor = 0.20        # local resonator radius factor
abs_thickness     = 1.0 * (math.pi / k_r)  # anchored absorber thickness
...
def solve_helmholtz(mesh, port_marker, freq, pml_offset=0.0, amp=1.0):
    ...
    x0         = mesh.coordinates().max(0) - pml_thickness - pml_offset
    sigma      = Expression(
    ...
    a_r += -(kr**2 - k_i**2)*(u_r*v_r - u_i*v_i)
    a_r +=  2*kr*k_i*(u_r*v_i + u_i*v_r)
    a_i = dot(grad(u_r), grad(v_i)) + dot(grad(u_i), grad(v_r))
    a_i += -(kr**2 - k_i**2)*(u_r*v_i + u_i*v_r)
    a_i += -2*kr*k_i*(u_r*v_r - u_i*v_i)
...
ki_sim = -slope
print("Fitted ki = {0:.3e} m^-1, ~{1:.3f} dB/mm".format(ki_sim, 20*math.log10(math.e)*ki_sim*1e-3))
ps_db = 20*np.log10(p/p[0])
plt.figure(); plt.semilogy(xs*1e3,p,'-o',ms=3); plt.xlabel('mm'); plt.ylabel('|p|'); plt.grid(True)
plt.savefig('results/attenuation.png'); plt.close()
plt.figure(); plt.plot(xs*1e3,ps_db,'-s',ms=3); plt.xlabel('mm'); plt.ylabel('Atten dB'); plt.grid(True)
plt.savefig('results/attenuation_db.png'); plt.close()

# 2) 3D stacked barrier simulation
barrier_length = columns * tuned_pitch
domain_length_bak = domain_length

domain_length = barrier_length
mesh2_list = []
isol = []
for gap in [5e-6,10e-6,20e-6,50e-6]:
    height = 2*waveguide_height + gap
    geom = Rectangle(Point(0,0), Point(domain_length, height))
    # Anchored absorbers at ends\    geom -= Rectangle(Point(0,0), Point(abs_thickness, height))
    geom -= Rectangle(Point(domain_length-abs_thickness,0), Point(domain_length, height))
    for layer in range(layers):
        ...
        for i in range(columns):
            ...
            elif i > columns-1-taper_count:
                rf_scale = float(columns-i)/taper_count
            ...
            for j in range(rows_per_layer):
                ...
                geom -= Circle(Point(x,y), rf)                         # Bragg hole
                geom = geom + Circle(Point(x,y), res_radius_factor*tuned_pitch)  # hybrid resonator
    mesh2 = generate_mesh(geom, 400)
    u2_r,u2_i = solve_helmholtz(mesh2, left_port, f0, pml_offset=tuned_pitch)
    V2 = FunctionSpace(mesh2,'CG',1)
    bnd=MeshFunction('size_t',mesh2,mesh2.topology().dim()-1); bnd.set_all(0)
    out1 = type('O1',(SubDomain,),{'inside':lambda self,x,on: on and near(x[0],domain_length) and x[1]<=waveguide_height+gap/2})()
    out2 = type('O2',(SubDomain,),{'inside':lambda self,x,on: on and near(x[0],domain_length) and x[1]>=waveguide_height+gap/2})()
    ...
    iso = 20*math.log10(P2/P1) if P1>0 else -np.inf; isol.append(iso)
print("Gap (um), Iso(dB)")
for g,i in zip([5,10,20,50],isol): print("{0:.1f}, {1:.2f}".format(g,i))
plt.figure(); plt.plot([5,10,20,50],isol,'-s',ms=5); plt.xlabel('µm'); plt.ylabel('Iso dB'); plt.grid(True)
plt.savefig('results/crosstalk.png'); plt.close()

domain_length = domain_length_bak

# 3) FFT & bandgap estimation
Lx,Ly = 6e-3,2e-3; geom3 = Rectangle(Point(0,-Ly/2),Point(Lx,Ly/2))
for i in range(int(Lx/tuned_pitch)): geom3 -= Circle(Point(tuned_pitch/2+i*tuned_pitch,0), radius_factor*tuned_pitch)
...
sp=np.abs(np.fft.fftshift(np.fft.fft(vals*np.hanning(len(vals)),8192)))
freqs=np.fft.fftshift(np.fft.fftfreq(8192,d=xs3[1]-xs3[0]))
plt.figure(); plt.plot(freqs,sp); plt.axvline(k_r,ls='--'); plt.axvline(-k_r,ls='--'); plt.grid(True)
plt.savefig('results/metamaterial_fft.png'); plt.close()
peak=sp.max(); thr=peak/math.sqrt(2)
mask=sp<thr
if mask.any(): kg=freqs[mask]; bw=kg.max()-kg.min(); df=bw*vs/(2*math.pi)
    print("Bandgap k-width={0:.3e} 1/m, freq stop-band≈{1:.3f} MHz".format(bw,df*1e-6))

# 4) Transmission sweep
freqs_sweep=np.linspace(f0*0.8,f0*1.2,41)
trans=[]
for fr in freqs_sweep:
    L4=columns*tuned_pitch
    geom4=Rectangle(Point(0,0),Point(L4,2*waveguide_height+20e-6))
    for i in range(columns): geom4-=Circle(Point(tuned_pitch/2+i*tuned_pitch,waveguide_height+10e-6),radius_factor*tuned_pitch)
    ...
    out4=type('O4',(SubDomain,),{'inside':lambda self,x,on: on and near(x[0],L4)})(); out4.mark(b4,1)
